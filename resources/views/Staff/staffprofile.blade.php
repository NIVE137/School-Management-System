...
<div class="profile-hero" style="position:relative;z-index:1;">
    <div class="d-flex align-items-center gap-4" style="position:relative;z-index:2;">
        @if($staff->image)
            <img src="{{ asset('asset/img/'.$staff->image) }}" alt="Profile" class="profile-avatar">
        @else
            <div class="profile-avatar-placeholder"><i class="fas fa-user"></i></div>
        @endif
        <div>
            <h4 class="fw-bold mb-1" style="color:#fff;">{{ $staff->staff_name }}</h4>
            <p style="color:rgba(255,255,255,0.75);margin:0;font-size:0.88rem;">{{ $staff->role_name }}</p>
            <span style="display:inline-flex;align-items:center;gap:5px;background:rgba(255,255,255,0.15);border-radius:20px;padding:3px 12px;font-size:0.73rem;font-weight:600;color:#fff;margin-top:8px;">
                <span style="width:7px;height:7px;border-radius:50%;background:{{ $staff->status=='active' ? '#71dd37' : '#ff3e1d' }};display:inline-block;"></span>
...
<div class="profile-card">
    <div class="section-sep">Staff Information</div>
    <div class="row">
        <div class="col-md-6"><div class="profile-field"><label>Staff Name</label><span class="val">{{ $staff->staff_name }}</span></div></div>
        <div class="col-md-6"><div class="profile-field"><label>Staff ID</label><span class="val">{{ $staff->staff_id }}</span></div></div>
        <div class="col-md-6"><div class="profile-field"><label>Mobile</label><span class="val">{{ $staff->mobile }}</span></div></div>
        <div class="col-md-6"><div class="profile-field"><label>Email Address</label><span class="val">{{ $staff->email }}</span></div></div>
        <div class="col-md-6"><div class="profile-field"><label>Role</label><span class="val">{{ $staff->role_name }}</span></div></div>
        <div class="col-md-6"><div class="profile-field"><label>Address</label><span class="val">{{ $staff->address }}</span></div></div>
        <div class="col-md-6"><div class="profile-field"><label>Date of Birth</label><span class="val">{{ $staff->dob }}</span></div></div>
    </div>
</div>
@endsection
