...
<div class="page-back">
    ...
    <div>
        ...
        <small class="text-muted">Update {{ $student->student_name }}'s record</small>
    </div>
</div>
...
<div class="custom-card">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('uploaddocuments',$student->id) }}" class="btn-cyan btn-sm" style="font-size:0.8rem;padding:7px 14px;"><i class="fas fa-file-upload me-1"></i> Upload Documents</a>
    </div>

    <form id="updateForm" action="{{ route('updatestudent', $student->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @if($student->image)
        ...
        <div class="row g-4 mb-4">
            ...
            <div class="col-md-6">
                ...
                <select name="class_name" class="form-select select-custom" required>
                    ...
                    @foreach($classes as $class)<option value="{{ $class->class_name }}" {{ $student->class_name==$class->class_name?'selected':'' }}>{{ $class->class_name }}</option>@endforeach
                </select>
            </div>
        ...
        <div class="d-flex gap-3 pt-2">
            ...
            <a href="{{ route('studentmanagement') }}" class="btn-discard">Discard</a>
        </div>
    </form>
</div>
