...
<div class="page-back">
    <a href="{{ route('studentmanagement') }}" class="btn-back"><i class="fa-solid fa-angle-left"></i></a>
    <div>
        <h4 class="fw-bold mb-0">Manage Classes</h4>
        <small class="text-muted">Create and manage classroom categories</small>
    </div>
</div>

<div class="custom-card">
    <div style="font-size:0.72rem;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#a5b7c8;margin-bottom:16px;padding-bottom:8px;border-bottom:2px solid #e7e7ff;">Create New Class</div>
    <form action="{{ route('storeclass') }}" method="POST">
        @csrf
        <div class="row g-3 align-items-end">
            <div class="col-md-10">
                <label class="form-label-custom">Class Name</label>
                <input type="text" name="class_name" class="form-control input-custom" placeholder="e.g. Grade 1, Class 10-A" required>
            </div>
            <div class="col-md-2">
                <button type="submit" class="btn-cyan w-100"><i class="fas fa-plus me-1"></i> Create</button>
            </div>
        </div>
    </form>
</div>
...
<div class="custom-card">
    <div class="table-responsive">
        <table class="table table-custom table-borderless align-middle w-100" id="class-table">
            <thead>
                <tr>
...
$(document).ready(function () {
    $('#class-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: { url: '{{ route("createclass") }}', type: 'GET' },
        columns: [
...
function deleteClass(id) {
    Swal.fire({
        title: 'Delete Class?', text: 'This cannot be undone.', icon: 'warning',
        showCancelButton: true, confirmButtonColor: '#ff3e1d', cancelButtonColor: '#566a7f',
        confirmButtonText: 'Yes, Delete'
    }).then(function(r) {
        if (r.isConfirmed) {
            $.ajax({
                url: '/Admin/deleteclass/' + id, type: 'DELETE',
